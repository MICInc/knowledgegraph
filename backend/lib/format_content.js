...
module.exports = {
	...
	check_edges: function(edges) {
		...
		for(var i in edges.split(', ')) {
			if(i.length > 0) {
				db.Content.findOne({ title: i }, function(err, article) {
					if(err || article == null) urls.push({ innerText: i, id: '', href: '' });
					else urls.push({ id: article._id, innerText: i, src: module.exports.generate_url(i) });
				});
			}
		}
